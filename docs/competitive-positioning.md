...
| Performance by day of week | Tradervue, Edgewonk | Fixed Sun-Sat calendar-order table (not sorted by P&L) showing win rate/expectancy/total P&L per weekday, computed from trade dates already logged |
| Performance by direction / by market | Tradervue, Edgewonk | Long vs Short and Stock/Options/Futures/Crypto breakdowns, reusing both fields already captured on every trade |
